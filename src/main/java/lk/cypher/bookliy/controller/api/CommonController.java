...
public class CommonController {
    ...
    @Path("/remove-cart-item/{cartId}")
    @DELETE
    @Produces(MediaType.APPLICATION_JSON)
    public Response removeCartItem(@PathParam("cartId") String cartId, @Context HttpServletRequest request) {
        String responseJson = new CommonServices().removeCartItem(cartId, request);
        return Response.ok().entity(responseJson).build();
    }
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response sortBy(@QueryParam("sortBy") String sortBy) {
        String responseJson = new CommonServices().getSortedProducts(sortBy);
        return Response.ok().entity(responseJson).build();
    }
